		...
		<script>
			...
			jQuery(document).ready(function() {
				...
				$("#billNo").keyup(function(){
					...
					//alert(billNo);
					$.post("<?php echo site_url("stockAjax/editStock") ?>",{billNo : billNo}, function(data){
						$("#saleDetail").html(data);
					});
					
				});
				$("#billNo12").keyup(function(){
					var billNo = $("#billNo12").val();
					//alert(billNo);
					$.post("<?php echo site_url("feeControllers/printDueById") ?>",{billNo : billNo}, function(data){
						$("#saleDetail").html(data);
					});
				...
				function stuBirthPlace() {
                    var text_value = document.getElementById("billNo12").value;
                    value = text_value.replace(/[ ]+/g," ").replace(/[^(A-Za-z0-9 )]*/g, "");
                    document.getElementById("billNo12").value=value;
