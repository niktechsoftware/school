...
<?php if( $classid->class_id == 262 && $row2== "D" || $classid->class_id == 343 && $row2== "D" || $classid->class_id == 261 && $row2== "D" || $classid->class_id == 342 && $row2== "D"){ ?>
<body style="background: none;" >	
<!---samta 9,10,11,12 body end--->	
<?php }else{ ?>
<!---other body stART--->
<body>
<!---other body end--->
       ...
       <?php if( $classid->class_id == 262 && $row2== "D" || $classid->class_id == 343 && $row2== "D" || $classid->class_id == 261 && $row2== "D" || $classid->class_id == 342 && $row2== "D"){ ?>
        ...
        <tr style="border-top:none;">
            <td class="center" style="border:none;">
                ...
                </br><?php if($info->registration_no>0){?><i>Aff.No. - <?php echo $info->registration_no;?><?php }?></i>
                </td>
...
$cumulativetotal=0;
       ...
       $pi=1;
		...
		<tr class="wight"> 
				...
				?></td>
			   <td class="center bold" colspan="2"><?php //echo calculateGrade($gtptal_grand,$classid->class_id)?></td>
   ...
   <!---2nd term exam name start--->
		...
		<tr class="wight"> 
               ...
               $ctotalmo[$t]+=$exammm;
				...
				<?php }else{ ?>
				 <?php  $hs =1; foreach ($examid->result() as $value):?>
					<td class="center" colspan="1" >
					<?php       $this->db->where("term", 1);
								$this->db->where('subject_id',$sub['subject']);
								$this->db->where('class_id',$classid->class_id);
               ...
               $ctotalmo2[$t]+=$exammm;
				...
				<?php $i++; $t++;endforeach; ?><td colspan="1" ></td>
				<?php }else{ ?>
				 <?php $hs = 1;foreach ($examid_2->result() as $value):?>
					<td class="center" colspan="1" >
					<?php       $this->db->where("term", 2);
								$this->db->where('subject_id',$sub['subject']);
								$this->db->where('class_id',$classid->class_id);
               ...
               $ctotalmo2[$t]+=$exammm;
				...
				<?php } ?>
					...
					<!--2nd term exam end-->
				<td class="center bold"><?php  if(($subjectname->id == 282) || ($subjectname->id == 288)){$rty = ($gtptal+$gtptal_2)/8;}else{ $rty = ($gtptal+$gtptal_2)/2;} echo $gtptal+$gtptal_2;  ?></td>
			   <td class="center bold"><?php $colmtot = $colmtot+ $gtptal+$gtptal_2;echo calculateGrade($rty,$classid->class_id)?></td>	
		</tr>
					<?php } }endforeach;?>
		<tr class="wight">
					<td class="subject">GRAND TOTAL</td>
			...
			<td class="center bold"><?php echo $colmtot;?></td>
			<td class="center bold"></td>
		</tr>
		...
		<tr class="wight" > 
			...
			$ctm =0;	$hy =1;	foreach ($examid->result() as $value):?>
					<td class="center" colspan="1">	
					<?php  		$this->db->where("term", 1);
            					$this->db->where('subject_id',$sub['subject']);
            					$this->db->where('class_id',$classid->class_id);
				...
				<?php  calculateGrade($rty,$classid->class_id);
...
<td class="center bold"><?php     ?></td>				
		...
		<tr class="blue">
			...
			<td colspan="4">RANK :<?php //echo $this->exammodel->getClassRank($studentInfo->id, $classid->class_id, $fsd);?></td>
		</tr>
		<!---SL marks table END--->
