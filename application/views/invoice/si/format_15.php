    	...
    	?>
			...
			<?php } ?>
				...
				<?php   if($examid_2->num_rows()>0){ ?>
					...
					$this->db->where('class_id',$classid->class_id);
					$this->db->where('exam_id',$value->exam_id);
			$exammm_row=	$this->db->get('exam_max_subject');
			if($exammm_row->num_rows()>0){
				$exammm=	$exammm_row->row()->max_m;
			}    
			if(is_numeric($exammm)){
					  $ttal=$ttal+$exammm;
				    $dhtm=$exammm+$dhtm;
    	                ...
    	                <div style="width:50%; float:left;">
    						...
    						<tr>
    							...
    							<tr>
    	                            ...
    	                            <td><?php  if($per >= 61 && $per < 81 ){ echo "B";}else{if($dhtm>0){echo $gradecal =co_scolastic($per,$classid->class_id);}}  ?></td>      
    	                        </tr>
    							<tr>
    	                            <td>Art Education</td>
    	   ...
    	   <br>
    	   <br>
    <?php 	function calculateGrade($val,$classid){
								if($val >= 91 && $val < 101):
									return 'A1';
