...
<?php $school_code=$this->session->userdata("school_code");?>
		 <?php 
            ...
            foreach($subname->result() as $subject){
                   ...
                   $subjectid=$subject->id;
                    $val=$this->db->query("select * from exam_max_subject WHERE exam_id = '$examid' AND class_id='$classid' AND subject_id='$subjectid' sub_type='$subtypeid' order by id");
                    // print_r($val->result());exit;
                    if($val->num_rows()>0){
            ?>
            <tbody>
            ...
            $examname=$this->db->get('exam_name')->row()->exam_name;
                 ...
                 <td><button class="btn btn-red" id="updtmmarks<?php echo $i;?>">Update Your Marks<i class="fa fa-arrow-circle-right"></i>
                  ...
                  <input type="hidden" id="examid1<?php echo $i;?>" value="<?php echo $examid; ?>" />
                      <input type="hidden" id="classid1<?php echo $i;?>" value="<?php echo $classid; ?>" />
                      <input type="hidden" id="subtypeid" value="<?php echo $subtypeid; ?>" />
...
<tr>
              ...
              <br><br>
                <?php 
              $val=$this->db->query("select * from exam_max_subject WHERE exam_id = '$examid' AND class_id='$classid' AND subject_id='$subjectid' and sub_type='$subtypeid' order by id ");
              		// print_r($val->result());exit;
              		?>
               <table class="table table-striped table-bordered table-hover" id="sample-table-2">
                <thead>
