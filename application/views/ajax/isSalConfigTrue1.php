...
$afd = $this->db->query("SELECT created FROM `emp_salary_struct` WHERE emp_id='$eid' ");
$temp = $this->db->query("SELECT * FROM emp_salary_info WHERE emp_id = '$eid' ORDER BY id DESC LIMIT 1");
$tfsd=$temp->row()->fsd;
$a = $this->db->query("SELECT SUM(monthNo) as month,fsd,created FROM `emp_salary_info` WHERE emp_id='$eid' and fsd='$fsd' ");
...
if(($month>0)&&($month<13)){
												...
												if($a1->created>$this->session->userdata("fsd")){
												    	$fsd = $a1->created;
												}
												else{
												    	$fsd=$this->session->userdata("fsd");
		...
		<input type="hidden" name = "empid" value ="<?php echo $eid; ?>" class="form-control" />
			...
			<div class="row">
				<div class="col-md-12">
                    ...
                    <div class="progress">
                       	<input type="hidden" name="fsd" value="<?php echo $a->row()->fsd; ?>" />
			...
			<div class="row">
				...
				<div class="col-md-3">
					<div class="form-group">
						...
						<select multiple="multiple" id="sld" name="diposit_month[]" class="form-control search-select" required="required">
                            <?php 
                            $month = $a->row()->month;
                           // $a1=$afd->row();
                            if($month<13 && $month>0){ 
                              ...
                              if(($month>0)&&($month<13)){
                                ...
                                for($i=$month; $i<=12; $i++):?>
                                <option id ="sld" value="<?php echo date("M-y",strtotime("$fsd + $i month"));?>">
                                     <?php echo date("M-Y",strtotime("$fsd + $i month"));?>
                                 </option>
                            <?php endfor; }
                            else{
			...
			<div class="row">
				...
				<div class="col-md-3">
					...
					<div class="form-group">
						...
						$absent = $this->teacherModel->countAttTeacher($edate,$sdate,$enum);
								  							?>
				...
				<div class="col-md-3">
					...
					<div class="form-group">
						...
						<?php $rrr=$gs-$tdf?>
						<input type="hidden" id="gross_s1" value ="<?php echo $rrr;?>" />
						<input type="text" name="gross_s" id="grossSalary" value ="<?php echo $rrr;?>" class="form-control"/>
					</div>
				</div>
			</div>
